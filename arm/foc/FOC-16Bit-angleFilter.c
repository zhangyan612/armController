...
signed char sensor_direction=CW; // ?? signed char ???????
...
float Position_KP		=1.773,Position_KI=0.177,Position_KD=0.089;          /* ???PID?? */
...
// Add this structure definition at the top of your code
typedef struct {
    float *values;
    int size;
    int index;
...
// Function to create a sliding filter
Filter create_sliding_filter(int size) {
    Filter filter;
    filter.values = (float *)malloc(size * sizeof(float));
    filter.size = size;
    filter.index = 0;
    filter.sum = 0.0f;
...
// Function to update the sliding filter with a new value
float Sld_Filter(Filter *filter, float new_value, int window_size) {
    // If window_size is different from filter size, adjust the filter
    if(window_size != filter->size) {
        free(filter->values);
        filter->values = (float *)malloc(window_size * sizeof(float));
...
// Function to free filter memory when done
...
float getAngle(void)
{
    ...
    float raw_angle = full_rotation_offset + current_angle;
    
    // Apply sliding filter to reduce noise
    float filtered_angle = Sld_Filter(&angle_filter, raw_angle, 10);
    ...
    if (pattern == 0)
    {
        ...
        pattern=4;
			  ...
			  // Initialize angle filter
				angle_filter = create_sliding_filter(10); // 10-point moving average
				velocity_filter = create_sliding_filter(5); // 5-point moving average for velocity
        ...
        case 3:
				...
				case 4:
					  // Temporary debugging output
						...
						P_angle.P = 1.0;  // Reduced from 5.0 to prevent saturation
						P_angle.I = 0.05;  // Add small integral term to overcome stiction
						P_angle.D = 0.05;  // Add derivative term for damping
						...
						voltage.q = _constrain(voltage.q, -10.0f, 10.0f);
...
void SysTick_1ms()
{
    ...
    if(count++>=100)
    {
        ...
        // Apply sliding filter to velocity
        shaft_velocity = Sld_Filter(&velocity_filter, shaft_velocity, 5);
    }
    // UART1 ??????
