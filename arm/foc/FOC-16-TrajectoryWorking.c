...
void updateTrajectoryMode() {
    ...
    // Only check encoder position to determine if we've reached the target
    float remaining = fabs(trajectory.target_angle - shaft_angle);
    
    // Check if we've reached the target position
    if (remaining <= 0.01f) {
        trajectory.current_speed = 0.0f;
        
        // Apply holding torque unless motor is disabled
        if (motor_enabled) {
            // Use position PID to hold position
            P_angle.P = 4.0f;  // Lower P gain for holding to prevent oscillation
            P_angle.I = 0.1f; // Small I term to maintain position
            P_angle.D = 0.1f;  // Small D term for damping
            
            float error = trajectory.target_angle - shaft_angle;
            voltage.q = PIDoperator(&P_angle, error);
            voltage.q = _constrain(voltage.q, -voltage_limit, voltage_limit); // Limit holding torque
            voltage.d = 0;
            
            // Keep updating electrical angle for proper field orientation
    ...
    // Continue with open-loop velocity profile as before
    ...
    if (trajectory.target_angle > shaft_angle) {
        // Need to increase angle (CW) - use negative speed
        trajectory.current_speed = -abs_current_speed;
    } else {
        // Need to decrease angle (CCW) - use positive speed
        trajectory.current_speed = abs_current_speed;
    }
