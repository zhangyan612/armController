...
typedef struct {
    ...
    uint8_t stage;            // Control stage (0:velocity mode, 1:position mode)  // New field
    float open_loop_angle;    // Open-loop electrical angle for velocity stage  // New field
} TrajectoryProfile;
...
// ?????????
uint8_t enable_current_output = 0;  // ????????
int current_output_counter = 0;     // ?????
#define CURRENT_OUTPUT_INTERVAL 10  // ????(?????)
...
PhaseCurrent_s getPhaseCurrents(void)
{
    ...
    for(int i = 0; i < SAMPLE_COUNT; i++) {
        ADC_1011 = _readADCVoltageInline();
        sum_a += (ADC_1011.adc_10 - 1.65) * 10;  // amps
        sum_b += (ADC_1011.adc_11 - 1.67) * 10;  // amps
    }
    
    current.a = sum_a / SAMPLE_COUNT;
    current.b = sum_b / SAMPLE_COUNT;
    current.c = 0;
...
void initTrajectoryMode(float speed, float accel, float revs)
{
    ...
    trajectory.active = 1;
    trajectory.stage = 0;  // Start in velocity stage
    trajectory.open_loop_angle = electrical_angle;  // Initialize open-loop angle
    ...
    printf("  Stage: %d (Velocity mode)\r\n", trajectory.stage);
}

// Update trajectory velocity mode
void updateTrajectoryMode()
{
    // Only update during velocity stage
    if (!trajectory.active || trajectory.stage != 0) return;
    ...
    switch (trajectory.phase) {
        ...
        case 2: // Deceleration phase
            ...
            if (trajectory.current_speed <= 0) {
                trajectory.current_speed = 0.0f;
                trajectory.stage = 1; // Switch to position stage
                target_angle = trajectory.target_angle;  // Set exact target position
                printf("Velocity stage completed, switching to position stage\r\n");
            }
            break;
    }
    ...
    switch(pattern)
    {
        ...
        case TRAJECTORY_MODE:  // Trajectory velocity mode (new)
            ...
            if (trajectory.active) {
                // Velocity stage: use open-loop speed control
                if (trajectory.stage == 0) {
                    // Calculate signed speed
                    float signed_speed = trajectory.direction * trajectory.current_speed;
                    
                    // Update open-loop angle
                    trajectory.open_loop_angle += signed_speed * pole_pairs * 0.001f;
                    trajectory.open_loop_angle = _normalizeAngle(trajectory.open_loop_angle);
                    
                    // Set electrical angle and voltage
                    electrical_angle = trajectory.open_loop_angle;
                    voltage.q = (signed_speed >= 0) ? 3.0f : -3.0f;
                    voltage.d = 0;
                    
                    printf("Traj velocity: %.2f rad/s\r\n", signed_speed);
                }
                // Position stage: use position control for fine adjustment
                else {
                    // Use position PID control
                    P_angle.P = 5;
                    P_angle.I = 0;
                    P_angle.D = 0.1;
                    voltage.q = PIDoperator(&P_angle, (shaft_angle - target_angle));
                    ...
                    float position_error = fabs(shaft_angle - trajectory.target_angle);
                    if (position_error < 0.01f) {  // 0.01 rad threshold
                        trajectory.active = 0;      // Mark trajectory complete
                        pattern = 4;               // Switch to position mode
                        printf("Position fine adjustment completed\r\n");
                    }
                }
            }
            break;
		...
		#if CURRENT_OUTPUT_ENABLED
    if(enable_current_output && motor_enabled)
    {
        if(current_output_counter++ >= CURRENT_OUTPUT_INTERVAL)
        {
            current_output_counter = 0;
            
            // ????????
            DQCurrent_s currents = getFOCCurrents(electrical_angle);
            
            // ?????: ??,??,Iq,Id
            printf("M:%d,S:%.2f,Q:%.3f,D:%.3f\n", 
                   pattern, 
                   shaft_velocity, 
                   currents.q, 
                   currents.d);
            ...
            case CMD_STATE_IDLE:
                ...
                if (received_char == 'J' || received_char == 'j') {
                    ...
                    printf("Remote calibration triggered: pattern=2\r\n");
								...
								if (received_char == 'O' || received_char == 'o') {
									enable_current_output = !enable_current_output;
									printf("Current output %s\n", 
												enable_current_output ? "ENABLED" : "DISABLED");
								}
