...
float getAngle(void)
{
    ...
    return total_angle * sensor_direction;
}
...
{    
	...
	switch(pattern)
	{
		case 2:
			...
			for(int i=0; i<=500; i++)
			{
				angle = _3PI_2 + _2PI * i / 500.0;  // Ç¶
				setPhaseVoltage(1, 0,  angle);  // Ó¦Ñ¹
				HAL_Delay(2);
			}
			mid_angle= getAngle();  // È¡Ð¼Ç¶
