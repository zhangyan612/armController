...
class RobStrideMotor:
    ...
    def _float_to_3bytes_from_le_tail(self, value: float) -> str:
        """
        协议要求：float 用小端 <f> 打包，然后取最后 3 字节（b[1:], 保持顺序），
        以空格分隔的十六进制小写字符串返回，例如：
        2.0 -> '00 00 40', 5.0 -> '00 a0 40'
        """
        b = struct.pack('<f', value)   # 小端 4 字节
        tail3 = b[1:]                  # 取最后 3 字节：索引 1,2,3
    ...
    def set_velocity(self, velocity: float = 2.0):
        ...
        cmd = f"41 54 90 07 e8 14 08 24 70 00 00 00 {val_hex} 0d 0a"
        self.send_command(cmd)
        time.sleep(0.1)
    ...
    def set_acceleration(self, acceleration: float = 5.0):
        ...
        cmd = f"41 54 90 07 e8 14 08 25 70 00 00 00 {val_hex} 0d 0a"
        self.send_command(cmd)
        time.sleep(0.1)

    def set_position(self, position: float):
        print(f"Setting position to {position}...")
        val_hex = self._float_to_3bytes_from_le_tail(position)
        cmd = f"41 54 90 07 e8 14 08 16 70 00 00 00 {val_hex} 0d 0a"
        self.send_command(cmd)
        time.sleep(0.1)
